...
import com.rongdai.activity.personalcenter.SetHandPasswordActivity;
import com.rongdai.utils.PersonInfoConstans;
import com.rongdai.utils.ScreenUtils;
...
public class Drawl extends View {
	...
	public Drawl(Context context, List<Point> list, String passWord,
		...
		bitmap = Bitmap.createBitmap(ScreenUtils.getScreenDispaly(context)[0], ScreenUtils.getScreenDispaly(context)[1], Bitmap.Config.ARGB_8888); // 设置位图的宽高
		canvas = new Canvas();
		canvas.setBitmap(bitmap);

		paint.setStyle(Style.STROKE);// 设置非填充
		paint.setStrokeWidth(10);// 笔宽5像素
		paint.setColor(Color.rgb(255, 18, 0));// 设置颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			SharedPreferences sp = context.getSharedPreferences(PersonInfoConstans.PERSONINFO, Context.MODE_PRIVATE);
			//如果手势密码是空的 则说明之前没有设置过手势密码  开始设置手势密码
			if(TextUtils.isEmpty(sp.getString(PersonInfoConstans.HNADPASSWORD, ""))){
				Editor editor = sp.edit();
				editor.putString(PersonInfoConstans.HNADPASSWORD, passWordSb.toString());
				editor.commit();
				SetHandPasswordActivity.sethand_title.setText("确认手势密码");
				SetHandPasswordActivity.sethand_info.setText("重新设置手势");
			}else{
				if(sp.getString(PersonInfoConstans.HNADPASSWORD, "").equals(passWordSb.toString())){
					Editor editor = sp.edit();
					editor.putString(PersonInfoConstans.HNADPASSWORD, passWordSb.toString());
					editor.commit();
					callBack.setSuccess();
	...
	private Point getPointAt(int x, int y) {
		int distands = ContentView.distands;
		for (Point point : list) {
			// 先判断x
			int leftX = point.getLeftX();
			int rightX = point.getRightX();
			if (!(x >= leftX - distands && x < rightX + distands)) {
				// 如果为假，则跳到下一个对比
				continue;
			}
			...
			if (!(y >= topY + distands && y < bottomY - distands)) {
				// 如果为假，则跳到下一个对比
				continue;
			}
