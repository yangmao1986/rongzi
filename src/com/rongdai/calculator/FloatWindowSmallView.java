...
//		percentView.setText(MyWindowManager.getUsedPercentValue(context));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (Math.abs(xDownInScreen - xInScreen) < viewHeight/4&& Math.abs(yDownInScreen - yInScreen)<viewHeight/4) {
				openBigWindow();
			}else{
				if(xInScreen>windowManager.getDefaultDisplay().getWidth()/2){
					mParams.x = (int) (windowManager.getDefaultDisplay().getWidth());
				}else{
					mParams.x = (int) (0);
				}
	...
	private void updateViewPosition() {
		...
		mParams.y = (int) (yInScreen - yInView)+25;
		windowManager.updateViewLayout(this, mParams);
	}
